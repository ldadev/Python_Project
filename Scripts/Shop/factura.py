...
def carrito():
	...
	while flag:
		try:
			enter =  int(input('Ingresa la cantidad de productos a agregar>>'))
			while enter > 0:
				enter-=1
				cod = str(random.randrange(100)) + random.choice(['a','e','i','o','u'])# Código aleatorio del producto
				producto,cantidad,costo_u = input('Ingresa el producto,cantidad y costo unitario>>').split(',')
				productos.setdefault(cod,[producto,int(cantidad),int(costo_u)])
			flag = False
